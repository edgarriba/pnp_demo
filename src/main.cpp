...
#include <opencv2/calib3d/calib3d.hpp>
 ...
 double params_QUICK_PRO[] = { width2*f2/sx2,   // fx
                               height2*f2/sy2,  // fy
                               width2/2,       // cx
                               height2/2};     // cy
 ...
 /*
   ...
   */
  double f3 = 43;
  double params_CANON2[] = { width*f3/sx,   // fx
                             height*f3/sy,  // fy
                             width/2,       // cx
                             height/2};     // cy
  ...
  PnPProblem pnp_verification(params_CANON2);
...
int main(int, char**)
{
  ...
  */
  ...
  // Create & Open Window
  cv::namedWindow("MODEL REGISTRATION 2", cv::WINDOW_KEEPRATIO);

  // Set up the mouse events
  cv::setMouseCallback("MODEL REGISTRATION 2", onMouseModelRegistration, 0 );

  // Open the image to register
  cv::Mat img_in2 = cv::imread(img_verification_path, cv::IMREAD_COLOR);
  ...
  while ( cv::waitKey(30) < 0 )
  {
    ...
    // Show the image
    cv::imshow("MODEL REGISTRATION 2", img_vis);
  }
  ...
  cv::Mat img2_gray;
  cv::cvtColor( img_in2, img2_gray, CV_RGB2GRAY );

  //-- Step 1: Calculate keypoints
  std::vector<cv::KeyPoint> keypoints_scene;
  orb.detect( img2_gray, keypoints_scene );

  //-- Step 2: Calculate descriptors (feature std::vectors)
  cv::Mat descriptors_scene;
  orb.compute( img2_gray, keypoints_scene, descriptors_scene );
